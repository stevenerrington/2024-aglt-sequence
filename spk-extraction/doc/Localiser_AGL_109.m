...
elseif AGL_type == 2
    sdir =  ['C:\Users\nlcn109\MATLAB\stimuli\AGL\stimuli\Test'];
end
...
elseif AGL_type ==2
    ...
    audiostims{3} = ['Grammatical5_novel'];
    audiostims{4} = ['Grammatical10_novel.wav'];
    audiostims{5} = ['Multiviolation_1.wav'];
    audiostims{6} = ['Multiviolation_2.wav'];
    audiostims{7} = ['Multiviolation_3.wav'];
    audiostims{8} = ['Multiviolation_4.wav'];
    audiostims{9} = ['Single_Rule_break_1.wav'];
    audiostims{10} = ['Single_Rule_break_2.wav'];
    audiostims{11} = ['Single_Rule_break_3.wav'];
    audiostims{12} = ['Single_Rule_break_4.wav'];
end
...
for trial = 1:par.numTrials
    ...
    par.fixDur      = par.stimOn + par.snddur(trial);     % when to turn off fixation (NEED TO CHANGE EVERY TRIAL DEPENDING ON SEQ LENGTH -YK)
    par.rewardDelay = par.fixDur;     % when to reward
    par.rewardDur   = 700;      % how long of a reward (from rewardDelay time)
    par.trialEnd    = par.rewardDelay+par.rewardDur;     % when does trial end
    ...
    if par.condOrder(trial) == 1, %A trial
        p = audioplayer(sounddata{par.stimOrder(trial)},dat.sound_samplefrequency);
        aud_delay_tmp = aud_delay{par.stimOrder(trial)};
        ...
        for e = 1:size(events,1)
            if GetSecs - trialStart > (events{e,2}/1000) && events{e,3} == 0
                ...
                elseif strcmp(events{e,1},'stimPlay')
                    ...
                    elseif par.condOrder(trial) > 1,
                        if par.condOrder(trial)==2, %V only index = -6
                            [pMov,movdur,fps] = Screen('OpenMovie', w, fullfile(sdir,movstims{par.stimOrder(trial)-6}));
                        elseif par.condOrder(trial)==3, %AV only index = -12
                            [pMov,movdur,fps] = Screen('OpenMovie', w, fullfile(sdir,movstims{par.stimOrder(trial)-12}));
                        end
                        Screen('PlayMovie', pMov, 1);
                        LJ_byte_out(7,ljHandle,LJ_ioPUT_DIGITAL_BIT);
                        dat.T.stimOn(trial) = GetSecs - trialStart ;
                        
                        % Playback loop: Runs until end of movie or keypress:
                        a = 0; last_tex = -1;
                        if ~exist('aud_delay_tmp')
                            aud_delay_tmp = 1;
                        end
                        aud_on = round(fps/(1/aud_delay_tmp)); % auditory delay in fps
                        while ~KbCheck
                            % Wait for next movie frame, retrieve texture handle to it
                            tex = Screen('GetMovieImage', w, pMov);
                            if (a == 0 && par.useAudDelay == 0 && par.condOrder(trial)==3) || (a == aud_on && par.useAudDelay == 1 && par.condOrder(trial)==3),
                                LJ_byte_out(5,ljHandle,LJ_ioPUT_DIGITAL_BIT);
                                play(p); % if Cond == 3, play sound with video now
                                dat.T.stimOn(trial) = GetSecs - trialStart ;
                            end
                            ...
                            Screen('FillRect', w, [255 255 255], [0 0 100 100]);
                            % Update display:
                            Screen('Flip', w);
                            % Release texture:
                            Screen('Close', tex);
                        ...
                        % Stop playback:
                        Screen('PlayMovie', pMov, 0);
                        % Close movie:
                        Screen('CloseMovie', pMov);
                        %                         LJ_byte_out(8,ljHandle,LJ_ioPUT_DIGITAL_BIT);
                    end
                ...
                elseif strcmp(events{e,1},'rewardOff') % && respType == 1
                    ...
                    LJ_byte_out(3,ljHandle,LJ_ioPUT_DIGITAL_BIT)
                    %%%TIMESTAMP%%%
