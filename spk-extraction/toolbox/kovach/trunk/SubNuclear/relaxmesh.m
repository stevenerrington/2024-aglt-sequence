...
niter = 10;
% ----------- SVN REVISION INFO ------------------
% $URL: https://saccade.neurosurgery.uiowa.edu/svn/KovachToolbox/trunk/SubNuclear/relaxmesh.m $
% $Revision: 842 $
% $Date: 2017-05-19 10:42:36 -0500 (Fri, 19 May 2017) $
% $Author: ckovach $
% ------------------------------------------------
...
edgd0 = sqrt(sum((template_mesh.X(edgi(:,1),:)-template_mesh.X(edgi(:,2),:)).^2,2));
   ...
   E = []
fa = facearea(newmesh);
 % constraint = 'fn'; % vertices may move along face normals (tends to cause the mesh to contract)
 constraint = 'vn'; % vertices may move along vertex normals
%  method = 'grad';
%  method = 'jitter';
 method = 'grad'
 njitt = 100;
