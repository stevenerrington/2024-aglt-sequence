function Localiser_optoAGL_109_V5(subInit,runNo)
...
%Last update: 2021-Sep-8 (YK)
% 2021-09-08: variable stimulation timing per sound, the fixation period is
% also prolonged.
...
par.fixDur      = par.stimOn + 2252 + 400 + 1000;     %%5th element + sound offset (400) + 1000 (stimulation duration)  Till the end of opto stimulationwhen to turn off fixation [413 976 1540 2103 2667] (NEED TO CHANGE EVERY TRIAL DEPENDING ON SEQ LENGTH -YK)
...
elseif AGL_type ==2
    ...
    par.snd_offset = 400; %This needs to be measured but currently set as 400ms
    par.opto_t = ([1126,2252,1126,2252,1126,2252,1126,2252,563,1126,563,563,1126,1126,2252,2252]+ par.snd_offset)/1000;
    %par.opto_t = ([1126,1126,1126,1126,1126,1126,1126,1126,1126,1126,1126,1126,1126,1126,1126,1126]+ par.snd_offset)/1000;
end
            ...
            if GetSecs - trialStart > (events{e,2}/1000) && events{e,3} == 0
                ...
                elseif strcmp(events{e,1},'stimPlay')
                    ...
                    if par.condOrder(trial) == 1, % if Audio only trial, just play sound
                        ...
                        %%opto trigger
                        WaitSecs('UntilTime', s + par.opto_t(par.stimOrder(trial)));
                        display(par.opto_t(par.stimOrder(trial)));
                        %LJ_byte_out(30,ljHandle,LJ_ioPUT_DIGITAL_BIT);
                        
                        %random variable to detect which trigger to put
                        freq = randi(2)-1;
                        if freq == 0 %Theta
                            Error = ljud_ePut(ljHandle,LJ_ioPUT_DAC,1,5,0);
                            LJ_byte_out(30,ljHandle,LJ_ioPUT_DIGITAL_BIT);
                        elseif freq == 1 %Gamma
                            Error = ljud_ePut(ljHandle,LJ_ioPUT_DIGITAL_BIT,3,1,0);
                            LJ_byte_out(31,ljHandle,LJ_ioPUT_DIGITAL_BIT);
                        end
