...
switch signal
    case 'spk'
        ...
        NCSpath = [fullfile(ops.dirs.raw_data,ops.filename,filepart_name) '.ncs'];

        spk_ncs_in = readncs([filepart_name '.ncs'],fullfile(ops.dirs.raw_data,ops.filename));
        timestamp(1,1) = spk_ncs_in.TimeStamp(1);
        timestamp(1,2) = spk_ncs_in.TimeStamp(end);        

    case 'events'
        [evnt_raw] = ft_read_event_BA(fullfile(ops.dirs.raw_data,ops.filename,['Events_' ops.session_n '.nev']));
