...
try
    [S,NlogL,optimInfo] =...
        gmcluster(gpuArray(X),k,start,reps,CovType,SharedCov,RegV,options,probtol);
    NlogL = double(gather(NlogL));
    S.PComponents = double(gather(S.PComponents));
    S.Sigma = double(gather(S.Sigma));
    S.mu = double(gather(S.mu));
