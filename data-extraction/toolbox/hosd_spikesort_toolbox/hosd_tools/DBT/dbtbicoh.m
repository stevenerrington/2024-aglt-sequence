                     ...
                     case {'nnb','nbb','bnb','mi'}
                       ...
                       %  upsamptx = 0;
                         ...
                         if isscalar(x2)
                          ...
                          dbx2 = dbt(x2,fs,opts.bw2,'remodphase',true,'upsampleFx',upsampf2,'lowpass',lpf,'gpu',false);
                     %     dbx2 = dbt(x,fs,bw2,'remodphase',false,'upsampleFx',opts.upsampfx);
                          dbx1 = dbt(x1,fs,opts.bw1,'upsampleFx',opts.upsampfx,'remodphase',true,'upsampleTx',upsamptx,'lowpass',lpf,'gpu',false);
                     ...
                     otherwise
                         ...
                         if isa(x1,'dbt')
                             dbx1 = x1;
                              ...
                              dbx1 = dbt(x1,fs,opts.bw1,'upsampleFx',opts.upsampfx,'remodphase',true,'upsampleTx',upsamptx*opts.bw2x-1,'lowpass',min(lpf,opts.w1lim),'gpu',false);
                         end
                          if isscalar(x2)
                            dbx2 = dbx1;
                         else
                            dbx2 = dbt(x2,fs,opts.bw2,'upsampleFx',opts.upsampfx,'remodphase',true,'upsampleTx',upsamptx-1,'lowpass',min(lpf,opts.w2lim),'gpu',false);
                         end
                 end
                     ...
                     case 'ww'
                         ...
                         if max(nch1,nch2) > 1
                            [W1,W2,WCh1,WCh2] = ndgrid(w1keep,1:opts.bw2x:sum(getf2),1:nch1,1:nch2);      
                             ...
                             [W1,W2] = ndgrid(w1keep,1:1:sum(getf2));
                             WCh1 = 1;
                             WCh2 = 1;
                         end
                         F3 = w1(W1)+w2(W2)-w2(1);
...
%                          blrep = reshape(blrep,size(blrep,1),size(blrep,2)*nch);
    ...
    %                           blrep2 = blrep;
    %                   end
                     BSP = blrep(:,I1(inds)).*blrep2(:,I2(inds)).* cblrep(:,I3(inds));
                     bspect(inds) = sum(BSP);
                    
                     if opts.stats
                       %Skipping every other window so that only non-overlapping windows are included in the analysis.
                       [out.stats.beta,out.stats.dev,pval,out.stats.iXX,out.stats.sigma,res,Yfit,df] = complexglm(BSP(2:2:end,:),[]);
                       out.stats.pval = nan(size(inds));
                       out.stats.pval(inds) = pval;
