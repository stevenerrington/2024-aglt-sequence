function [out,pspindices,stat] = pspect(S,varargin)
...
switch options.normalization
    ...
    case 'polycoh'        
        nrm = sqrt(sum(abs(NORM(:,:,1)).^2).*sum(NORM(:,:,2)),1);
        nrm(end+1)=1;
        out.normalization = nrm(rmat);
        out.bias=nan;
    case 'rms'        
...
if nargout >2
   [b,dev,pval,iXX,sigma,res,Yfit,df,sigmarel] = complexglm(PS,[],'diagonly',false);
   b(end+1) = nan;
   pval(end+1) = nan;
   sigma(end+1) = nan;
   sigmarel(end+1)=nan;
   ...
   stat.pval = pval(rmat);
   stat.iXX  = iXX;
   stat.se = iXX*sigma(rmat);
   stat.serel = sigmarel(rmat); %To find variance of real and imaginary parts, Re{stat.se+stat.srel}/2 and Re{stat.se - stat.srel}/2, respectively.
end
