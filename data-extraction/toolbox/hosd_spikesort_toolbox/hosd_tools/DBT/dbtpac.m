...
for k = 1:length(dbamp.frequency)   
    ...
    if get_trf
        [PAC(:,:,k,:,:,:),c,phfreq,tt,dbph,trf(:,:,k,:,:),~,~,Pval(:,:,k,:,:)] = dbtcoh(Xrs,scalar_trans(squeeze(abs(dbamp.blrep(:,k,:)))),ampfs,phargs{:},cohargs{:});
    elseif do_permtest
         [PAC(:,:,k,:,:,:),c,phfreq,tt,dbph,trf(:,:,k,:,:),~,Pperm(:,:,k,:,:)] = dbtcoh(Xrs,scalar_trans(squeeze(abs(dbamp.blrep(:,k,:)))),ampfs,phargs{:},cohargs{:});        
    else
        [PAC(:,:,k,:,:,:),c,phfreq,tt,dbph,~,~,~,Pval(:,:,k,:,:)] = dbtcoh(Xrs,scalar_trans(squeeze(abs(dbamp.blrep(:,k,:)))),ampfs,phargs{:},cohargs{:});        
    end
    if get_csd
        csd(:,:,k,:,:,:) = c;
    end
