...
% E = (PX*inv(FI)*PX');
if size(X,2) > 1
    errfun = @(varargin) sqrt(sum(ps.function(varargin{1})'.*(inv(FI)*ps.function(varargin{1})'),1))'; %Returns the ML estimation error for the given contrast
%     errfun = @(varargin) sqrt(diag(ps.function(varargin{1})*inv(FI)*ps.function(varargin{1})')); %Returns the ML estimation error for the given contrast
    estfun = @(varargin) ps.function(varargin{1})*P; %Returns the estimation given contrast
    errmat = @(varargin) ps.function(varargin{1})*inv(FI)*ps.function(varargin{1})'; %Returns the ML estimation error for the given contrast
else
    errfun = @(varargin) sqrt(sum(ps.function(varargin{1})'.*(inv(FI)*ps.function(varargin{1})'),1))'; %Returns the ML estimation error for the given contrast
%     errfun = @(varargin) sqrt(diag(ps.function(varargin{1})*inv(FI)*ps.function(varargin{1})')); %Returns the ML estimation error for the given contrast
    estfun = @(varargin) ps.function(varargin{1})*P; %Returns the estimation given contrast
...
% stats.parfun = @(varargin) ps.function(varargin{1});
stats.parfun = @(varargin) ps.function(varargin{1});
