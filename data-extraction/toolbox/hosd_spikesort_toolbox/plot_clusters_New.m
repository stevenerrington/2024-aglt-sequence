...
if isfield(clust.params,'plotting') && clust.params.plotting.plot_standardized && isfield(clust,'normalization')
    nrmlz = permute(clust.normalization(:),[2 3 1]);
else
    nrmlz = 1;
end

showt = clust.wint>spike_window(1) & clust.wint< spike_window(2);


% spw = single([clust.spike_sep.spike_waves])./nrmlz;
% qt = quantile(spw(:),[.0001 .9999]);
% qt = minmax(qt(:));
subplot(1,3,1)
[A,B] = ndgrid(1:a(1),1:a(2));
geometry = struct('x',num2cell(A(:)),'y',num2cell(B(:)));
dx = diff(minmax(clust.wint(showt)))*1000*1.1;
dy = -1.1;
for k = 1:nclust
%     subplot(a(1),a(2)*3,k + 2*a(2)*floor((k-1)/a(2)))
    sp = clust.spike_sep(k);
    
    sp.spike_waves = single(sp.spike_waves(showt,:,:))./nrmlz;
    
    M  = squeeze(mean(sp.spike_waves,2));
    [~,mxi] = max(max(M)-min(M));
    
    spw = sp.spike_waves(:,:,mxi)';
     qt = quantile(spw,[.01 .99]);
     qt = minmax(qt(:));
...
%     plot(clust.wint*1e3,sp.spike_waves(:,rp),'color',[1 1 1]*.5);
    imh = imagesc(clust.wint(showt)*1e3 + geometry(k).x*dx,bb+ geometry(k).y*dy ,H); 
    imh.AlphaData = 1-H(:,:,1);
    axis xy
    hold on, 
    plot(clust.wint(showt)*1e3 + geometry(k).x*dx,mean(sp.spike_waves(:,:,mxi),2)/nrm+geometry(k).y*dy,'linewidth',2,'color',cols(k,:));
    if size(M,2)>1       
        txh = text( geometry(k).x*dx,geometry(k).y*dy + bb(end), sprintf('%i (N=%i)\nmax ch%i',k,sp.cluster_count,mxi),'HorizontalAlignment','Center');%,'FontSize',6);
%         title(sprintf('%i (N=%i; max ch%i)',k,sp.cluster_count,mxi))
    else
        txh = text( geometry(k).x*dx,geometry(k).y*dy+bb(end),sprintf('%i (N=%i)',k,sp.cluster_count),'HorizontalAlignment','Center');%,'FontSize',6);        
%         title(sprintf('%i (N=%i)',k,sp.cluster_count))        
    ...
    axis tight 
    grid on
%     xlim([-1 1]*min([abs(xlim),5]));
%     ylim([-1 1]*max(abs(ylim)))
%     ylim([-1 1]*6*std(sp.spike_waves(:)));
end
plot( [0 5]+.9*dx/2,[1 1]*(geometry(k).y*dy+dy/3),'linewidth',2,'color','k')
text( 5/2 +.9*dx/2,geometry(k).y*dy+dy/3*1.1,'5','horizontalAlignment','center')
plot( [0 0]+.9*dx/2,[0 10/mean(nrmlz)]+(geometry(k).y*dy+dy/3),'linewidth',2,'color','k')
text( dx/2*.81,5/mean(nrmlz)+geometry(k).y*dy+dy/3,'10','horizontalAlignment','right')
axis off

%Projection to maximize separation.
wgt = cat(3,clust.spike_sep.cluster_count); %Weight SS by cluster count
wgt = wgt./sum(wgt);

MM = clust.Mx'*clust.Mx;
S = sum(clust.SS.*wgt,3);
[u,~] = svd(S^-1*MM);
...
if isfield(clust.spike_sep,'active_feature')
    ...
    xl = xlim;        
    for k = 1:size(A,1)
        txh(k) = text(xl(1)-.025*diff(xl),k,num2str(k),'HorizontalAlignment','right'); %#ok<*AGROW>
        if isfield(clust,'used_components') && clust.used_components(k)
            txh(k).FontWeight = 'bold';
            txh(k).FontSize = round(txh(k).FontSize*1.1);
        else
            txh(k).Color = [1 1 1]*.5;
        end       
    end
    ff = find(diff([0;srt(:);size(A,1)+1]));
    set(gca,'xtick',(ff(1:end-1)+ff(2:end))/2,'xticklabel',1:nclust,'ytick',1:size(A,1),'yticklabel','   ')
    axis 
