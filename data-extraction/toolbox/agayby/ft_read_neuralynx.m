function [data_all] = ft_read_neuralynx(cfg)
...
% if ~isfield(cfg,'latency') %default is take all samples if not specficied before calling the function   
%     cfg.samples = [1 orig.nSamples];
% else 
%     cfg.samples = [round(cfg.latency(1)*SamplingFrequency(1)) round(cfg.latency(2)*SamplingFrequency(1))];
% end

nbytes = fprintf('\nProcessing channel 0 ...\n');
for i = 1:nchans
    fprintf(repmat('\b',1,nbytes));
    nbytes = fprintf('\nProcessing channel %d...', i);
% ts    = ft_read_data(fname{i}, 'timestamp', true, 'begsample', cfg.samples(1), 'endsample', cfg.samples(2) );
