...
for timepoint_i = 1:n_times
    ...
    glm_output.trial_type.sig_times(1,timepoint_i) = u_t_mdl.Coefficients.pValue(2) < p_value_cutoff; % trial type
    glm_output.trial_type.beta_weights(1,timepoint_i) = u_t_mdl.Coefficients.tStat(2); % trial type
    
    % - Sound C
    glm_output.trial_type.sig_times(2,timepoint_i) = u_t_mdl.Coefficients.pValue(3) < p_value_cutoff; % trial type
    glm_output.trial_type.beta_weights(2,timepoint_i) = u_t_mdl.Coefficients.tStat(3); % trial type
    
    % - Sound D
    glm_output.trial_type.sig_times(3,timepoint_i) = u_t_mdl.Coefficients.pValue(4) < p_value_cutoff; % trial type
    glm_output.trial_type.beta_weights(3,timepoint_i) = u_t_mdl.Coefficients.tStat(4); % trial type
    
    % - Sound F
    glm_output.trial_type.sig_times(4,timepoint_i) = u_t_mdl.Coefficients.pValue(5) < p_value_cutoff; % trial type
    glm_output.trial_type.beta_weights(4,timepoint_i) = u_t_mdl.Coefficients.tStat(5); % trial type

    % - Sound G
    glm_output.trial_type.sig_times(5,timepoint_i) = u_t_mdl.Coefficients.pValue(6) < p_value_cutoff; % trial type
    glm_output.trial_type.beta_weights(5,timepoint_i) = u_t_mdl.Coefficients.tStat(6); % trial type

    % - Position 1
    p_value_cutoff = 0.001;
    glm_output.trial_type.sig_times(6,timepoint_i) = u_t_mdl.Coefficients.pValue(7) < p_value_cutoff; % trial type
    glm_output.trial_type.beta_weights(6,timepoint_i) = u_t_mdl.Coefficients.tStat(7); % trial type

    % - Position 2
    glm_output.trial_type.sig_times(7,timepoint_i) = u_t_mdl.Coefficients.pValue(8) < p_value_cutoff; % trial type
    glm_output.trial_type.beta_weights(7,timepoint_i) = u_t_mdl.Coefficients.tStat(8); % trial type

    % - Position 3
    glm_output.trial_type.sig_times(8,timepoint_i) = u_t_mdl.Coefficients.pValue(9) < p_value_cutoff; % trial type
    glm_output.trial_type.beta_weights(8,timepoint_i) = u_t_mdl.Coefficients.tStat(9); % trial type

    % - Position 4
    glm_output.trial_type.sig_times(9,timepoint_i) = u_t_mdl.Coefficients.pValue(10) < p_value_cutoff; % trial type
    glm_output.trial_type.beta_weights(9,timepoint_i) = u_t_mdl.Coefficients.tStat(10); % trial type

    % - Position 5
    glm_output.trial_type.sig_times(10,timepoint_i) = u_t_mdl.Coefficients.pValue(11) < p_value_cutoff; % trial type
    glm_output.trial_type.beta_weights(10,timepoint_i) = u_t_mdl.Coefficients.tStat(11); % trial type
