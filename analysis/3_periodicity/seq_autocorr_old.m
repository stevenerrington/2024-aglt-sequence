...
for clu_i = 1:6
    [acorr_auditory(clu_i,:), lags] = xcorr(nanmean(seq_sdf_out(intersect(neuron_class.cluster_idx.(['clu' int2str(clu_i)]),auditory_neuron_idx),acorr_time)), 'coeff');
    [acorr_frontal(clu_i,:), lags] = xcorr(nanmean(seq_sdf_out(intersect(neuron_class.cluster_idx.(['clu' int2str(clu_i)]),frontal_neuron_idx),acorr_time)), 'coeff');
end
